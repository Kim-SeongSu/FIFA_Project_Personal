...
def TCC_main():
    ...
    driver , A = __open_chrome(800,1400,1,3) # driver: 웹드라이버 , A: 랜덤 시간 지정
...
def __team_color_crawling_list(driver,A):
    ...
    for x in tqdm(range(2,7)):
            driver.find_element(By.XPATH, f'//*[@id="sForm"]/div[3]/div[2]/div[2]/div/div/div[{x}]/label').click()
            time.sleep(A)
            
            elements = TC_list_crawling(driver,A)
...
def __team_color_crawling_data(driver, A, teamcolor_type,teamcolor_club,teamcolor_nation,teamcolor_reinforce,teamcolor_relation,teamcolor_special):
    # 홈페이지 열기
    url = 'https://fifaonline4.nexon.com/datacenter/teamcolor?strTeamColorCategory=&strTeamColorType=&strTeamColorName='
    driver.get(url)
