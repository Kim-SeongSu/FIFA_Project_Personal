...
def __df_teamcolor_relation_plus(driver,A,teamcolor_relation):
    ...
    for x in tqdm(teamcolor_relation[:2]):
        try:
            # 팀 컬러 타입 선택
            driver.find_element_(By.XPATH, '//*[@id="sForm"]/div[3]/div[2]/div[2]/div/div/div[5]/label').click() 
            time.sleep(1)
            ...
            while min_num > 40:
                ...
                try:    
                    # 조건 만족하는 선수 숫자 세기
                    player_list = driver.find_element(By.XPATH, '//*[@id="ulPlayerList"]')
                    player_list_num = int(player_list[0].text.count('BP'))
                
                    # 팀 컬러 하나의 선수 목록을 저장할 리스트
                    temp_TC_relation_plus_while =[]
                    ...
                    for num in range(1,player_list_num+1):         
                        ...
                        print(total_temp)
                        temp_TC_relation_plus_while.append(total_temp)

                    temp_TC_relation_plus_for.append(temp_TC_relation_plus_while)
                
                except:
                    if driver.find_element(By.XPATH, '//*[@id="ulPlayerEmpty"]/li/div[2]').text == '검색된 선수가 없습니다.':
                        temp_TC_relation_plus_for.append('')
                    else:
                        break
                    
                
            TC_relation_plus.append(temp_TC_relation_plus_for)
